# 个人任务管理系统 — 应用说明与 API 概览

> **CLI + Web 双形态 · Inbox 收集分流 · Agenda 时间线 · 撤销回滚 · JSON/SQLite 双存储 · REST API v1**

一个功能完整的个人任务管理工具，使用 Python 开发，支持命令行与 Flask Web 两种使用方式，数据存本机、可安全备份。仓库总览见[上级 README](../README.md)。

---

## ✨ 特性总览

- ✅ 任务 CRUD、搜索过滤、统计报表和周报
- ✅ 任务提醒、每日摘要
- ✅ Today 工作台（今天 / 逾期 / 接下来 / 无日期四段式分组）
- ✅ **Inbox 收集箱**：先收集再规划，三种分流方式
- ✅ **Agenda 七天日程时间线**：三层筛选 + 鼠标 / 触控 / 键盘改期
- ✅ **统一任务快速操作与撤销回滚**：乐观锁防冲突
- ✅ **统一视图 Shell**：分组导航 + 全局搜索 + 移动端底部导航
- ✅ WeKan 式三列任务看板，状态拖拽持久化
- ✅ 显式项目与项目级任务隔离、可保存组合筛选视图
- ✅ 归档、恢复和安全备份导入
- ✅ JSON / SQLite 存储后端可切换
- ✅ REST API v1（认证、分页、健康检查、能力发现）
- ✅ 深色主题、移动端导航、可访问性基础
- ✅ GitHub Actions 质量门禁，45 个测试文件完整覆盖
...
然后在浏览器访问 `http://localhost:5000`。归档任务：`/archive`；保存视图：`/views`；备份和导入：`/settings`。
...
REST API 基础路径为 `/api/v1`。访问 `/api/v1` 可以查看当前 API 版本、认证要求、分页限制和端点发现信息；完整请求体、筛选参数和错误响应请参阅 [`docs/API.md`](docs/API.md)。
...
├── models/              # 数据模型（task / project / saved_view）
├── storage/             # 存储层（interface / factory / json_storage / sqlite_storage）
├── commands/            # CLI 命令（create / projects / list_tasks / update / delete / search / stats / remind / storage）
...
├── utils/               # 工具函数和安全脱敏（helpers / security）
├── scripts/             # 发布前维护脚本（security_scan / release_smoke）
├── docs/                # 接口契约（API.md / SCHEMA_MIGRATION_PLAN.md）
...
- Python + Flask（Web 服务与 CLI 共用存储工厂）
- JSON 本地存储（可选 SQLite，环境变量切换）
- pytest 回归测试 + GitHub Actions 质量门禁
