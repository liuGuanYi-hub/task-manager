...
- 任务 CRUD、Today 工作台和统一任务详情抽屉
- **Inbox 收集箱与任务分流**（安排到今天 / 指定日期 / 归入项目）
- **Agenda 七天日程时间线**（日期 / 密度 / 时间段三层筛选，鼠标拖拽、触控长按、键盘三种改期方式）
- **统一任务快速操作与撤销回滚**（完成 / 恢复 / 归档 / 延后 / 撤销，乐观锁防冲突）
- **统一视图 Shell**（分组导航、工作区工具栏、全局搜索、移动端底部导航）
...
- GitHub Actions、敏感信息扫描、45 个测试文件（100+ 回归测试）和 16 个路由 Blueprint
...
当前共有 45 个测试文件，覆盖任务模型、存储层和 0.1 ~ 16.6 各阶段功能（含 Inbox 分流、统一操作撤销回滚、Agenda 筛选与改期契约等）。
